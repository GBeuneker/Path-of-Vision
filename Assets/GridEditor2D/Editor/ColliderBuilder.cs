    ...
    public class ColliderBuilder : Editor
    {
        ...
        static void Optimize(Transform layerTransform)
        {
            ...
            foreach (Tile ff in tiles)
            {
                ...
                if (1 < brk)
                {
                    ...
                    if (ff.go.transform.localEulerAngles.y != 180)
                        ff.boxCollider.offset = new Vector2((ff.boxCollider.bounds.extents.x - boundX), ff.boxCollider.offset.y);
                    else
                        ff.boxCollider.offset = new Vector2(((ff.boxCollider.bounds.extents.x - boundX) * -1), ff.boxCollider.offset.y);
            ...
            foreach (Tile ff in tiles)
            {
                ...
                float boundY = ff.boxCollider.bounds.extents.y;
                colliderSizeY = 0;
                ...
                if (1 < brk)
                {
                    ...
                    if (ff.go.transform.localEulerAngles.y != 180)
                        ff.boxCollider.offset = new Vector2(ff.boxCollider.offset.x, (ff.boxCollider.bounds.extents.y - boundY));
                    else
                        ff.boxCollider.offset = new Vector2(ff.boxCollider.offset.x, ((ff.boxCollider.bounds.extents.y - boundY) * -1));
                }
            }
        }
